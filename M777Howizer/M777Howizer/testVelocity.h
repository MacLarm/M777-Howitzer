...
public:
   ...
   *********************************************/
   ...
   {
      // setup: Create two velocities
      Velocity vLHS(0.0, 0.0); // Left-hand side, stationary
      Velocity vRHS(2.3, 4.5); // Right-hand side with values (2.3, 4.5)

      // exercise: Add the velocities together
      vLHS.add(vRHS); // Or vLHS.add(vRHS) if you use an add method

      // verify: The result should be equal to vRHS, i.e., (2.3, 4.5)
      assertEquals(vLHS.getDX(), 2.3); // Check if the dx of result is 2.3
      assertEquals(vLHS.getDY(), 4.5); // Check if the dy of result is 4.5
   }
   ...
   *********************************************/
   ...
   {
      // Setup: Create two velocities
      Velocity vLHS(2.3, 4.5); // Left-hand side velocity
      Velocity vRHS(0.0, 0.0); // Right-hand side velocity (nothing)

      // Exercise: Add the velocities together
      vLHS.add(vRHS); // Or vLHS.add(vRHS) if you're using an add method

      // Verify: The result should be the same as vLHS
      assertEquals(vLHS.getDX(), 2.3); // Check if the dx of result is 2.3
      assertEquals(vLHS.getDY(), 4.5); // Check if the dy of result is 4.5
   }
   ...
   *********************************************/
   void addV_moving()
   {
      // Setup: Create two velocities
      Velocity vLHS(2.3, 4.5); // Left-hand side velocity
      Velocity vRHS(100.0, 200.0); // Right-hand side velocity (moving)

      // Exercise: Add the velocities together
      vLHS.add(vRHS); // Or vLHS.add(vRHS) if you're using an add method

      // Verify: The result should be the sum of vLHS and vRHS
      assertEquals(vLHS.getDX(), 102.3); // Check if the dx of result is 102.3
      assertEquals(vLHS.getDY(), 204.5); // Check if the dy of result is 204.5
   }
};
